...
void textbuffer_swap (Textbuffer tb, size_t pos1, size_t pos2) {
	...
	//Navigate to pos1
	line curr_pos1 = traverse(tb,pos1,0);

	//Navigate to pos2
	line curr_pos2 = traverse(tb,pos2,1);
...
void textbuffer_delete (Textbuffer tb, size_t from, size_t to) {
	...
	if(from>(textbuffer_lines(tb)-1) || to>(textbuffer_lines(tb)-1)) {
		fprintf (stderr, "Out of bounds\n");
		abort();
	}
...
Textbuffer textbuffer_copy (
	...
	if(from>(tb->n_items-1) || to>(tb->n_items-1)) {
		fprintf (stderr, "Out of bounds\n");
		abort();
	}
...
void textbuffer_replace (Textbuffer tb, char *match, char *replace) {
    ...
    while(ptr!=NULL){
		...
		//End buffer (Due to strncpy, not guaranteed to be NULL terminated)
        buffer[ptr-string] = '\0';

		//Concenate replacement
        strcat(buffer, replace);

        strcat(buffer, ptr+strlen(match));
...
//(A): Multiple consecutive newlines
	...
	assert(strcmp(new_line->last->text,"newline")==0);
