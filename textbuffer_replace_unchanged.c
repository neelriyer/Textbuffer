...
//TODO change this shit !!!!! (get rid of function potentially)
char *string_replace(char *string, char *match, char *replace){

	//if match and replace are the same
	if(strcmp(match,replace)==0) return string;
    ...
    while(ptr!=NULL){
		...
		int length = strlen(ptr+strlen(match));
...
size_t textbuffer_search (Textbuffer tb, char *match, bool rev) {
	...
	//return value (TODO: is this the best way to avoid warning?)
...
Textbuffer textbuffer_copy (
	...
	//if from/to out of range (TODO return; instead of NULL)
...
void textbuffer_replace (Textbuffer tb, char *match, char *replace) {
	...
	if(tb->n_items==0) return;

	//match is the same as replace
	//if(strcmp(match,replace)==0)return;

	//match is not found in Textbuffer
	if(textbuffer_search(tb,match,0)==-1) return;
...
//(A): Empty string
	...
	//test
	//Should be 1
	assert(textbuffer_bytes(empty) == 0);
...
printf("Test 11: Testing textbuffer_replace\n");
	...
	//(B): Textbuffer is empty
	//Use function
	empty = textbuffer_new("");
	textbuffer_replace (empty, "more","less");

	assert(strcmp(empty->first->text,"")==0);

	//(C): Textbuffer is null
	tb_null = textbuffer_new(NULL);
	textbuffer_replace (tb_null, "more","less");
